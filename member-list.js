...
async function run() {
  ...
  log('Opening Member List report...');
  await page.goto('https://app.clubready.com/Reporting/ReportViewer');
  await page.waitForLoadState('domcontentloaded');
  await page.waitForSelector('.menuItem65 a', { state: 'attached', timeout: 30000 });
  await page.evaluate(() => document.querySelector('.menuItem65 a').click());
  await page.waitForTimeout(2000);

  log('Running report...');
  await page.click('#showReport-31');
  ...
  const filename = `member-list-${date}.csv`;
